 ...
 */
void USART0_Transmit(unsigned char data){ //transmition pour le 0
while (!(UCSR0A & (1<<UDRE0)));/* on attend que la transmition soit finis */
UDR0 = data;/* Put data into buffer, sends the data */
}

void USART3_Transmit(unsigned char data3){ //transmition pour le 3
while (!(UCSR3A & (1<<UDRE3)));/* on attend que la transmition soit finis */
UDR3 = data3;/* Put data into buffer, sends the data */
}
 ...
 * cela marche aussi pour le 3 par le meme principe
 */
unsigned char USART0_Receive(void){//reception pour le 0
while (!(UCSR0A & (1<<RXC0)));
return UDR0;
}
unsigned char USART3_Receive(void){//reception pour le 0
while (!(UCSR3A & (1<<RXC3)));
return UDR3;
}
 ...
 * cela marche aussi pour le 3 par le meme principe
 */
ISR(USART0_RX_vect){ //interuption pour le 0
 // data0 = UDR0;
  flag0 =1;
//  PORTB = 0b00100000;
}
ISR(USART3_RX_vect){//interuption pour le 3
 /// data3 = UDR3;
  flag3 =1;
}
 ...
 * cette fonction utilise la fonction tansmetre jusqua ce quelle transmettre le code ASCII 00 (code qui est forcement transmi a la fin d'une phrase pour signifier quelle est finis)
 */
...
sign = '+';
value= '15';
//DDRB = 0b00100000; //pour la led
//PORTB = 0b0;
sei();
USART0_Transmit(way);
USART0_Transmit(sign);
